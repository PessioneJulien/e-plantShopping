...
const CartItem = ({ onContinueShopping, handleDeleteToCart }) => {
  ...
  const calculateTotalAmount = () => {
    let totalAmount = 0;
    cart.forEach(item => {
      // item.cost is a string with a $ sign so we need to delete the $ sign and convert it to a number
  ...
  const calculateTotalCost = (item) => {
    // cost is $x so we need to delete the $ sign and convert it to a number
    const cost = item.cost.replace('$', '');
